...
from database import init_db, log_traffic, get_recent_logs, get_recent_incidents
from utils.traffic_utils import calculate_speed, check_collision, IncidentManager
...
incident_manager = IncidentManager(cooldown=10)

def generate_frames():
    global current_source_mode, is_paused, last_processed_frame, unique_vehicles, track_history, speeds
    
    base_path = os.path.dirname(__file__)
    video_source = os.path.join(base_path, "assets", "traffic_video.mp4")
    
    active_mode = None
    cap = None
    
    last_log_time = time.time()
    # incident_cooldown is now managed by incident_manager
                        ...
                        if id in track_history:
                            ...
                            if len(track_history[id]) > 2:
                                ...
                                if id in speeds and speeds[id] > 20 and speed < 2:
                                    if not incident_manager.is_cooldown_active('SUDDEN_STOP'):
                                        ...
                                        incident_manager.trigger_incident('SUDDEN_STOP')
            ...
            for i in range(len(active_boxes)):
                for j in range(i + 1, len(active_boxes)):
                    if check_collision(active_boxes[i], active_boxes[j]):
                        if not incident_manager.is_cooldown_active('COLLISION'):
                            id_i = active_ids[i]
                            id_j = active_ids[j]
                            ...
                            incident_manager.trigger_incident('COLLISION')
                            stats["last_incident"] = desc
